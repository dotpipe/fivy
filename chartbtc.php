...
class CNGN
{
    ...
    function bitcoin(string $btc_csv, int $day_cnt, $data_column = 1, $date_column = 0)
    {
        ...
        $theaders = "<tr><td style='width:150;margin-top:5px;'>Long Form Date (Asc.) </td><td> Real Open </td><td> Chase Open </td><td> Real Adj. Close </td><td> Chase Close </td></tr>";
        $y = 1;
        $vals = [];
        $x = 0;
        $exp = 1;
        ...
        for ($i = count($seq) - 1; $i >= 0; $i--) {
            ...
            // while ($j < 2)
            {
                ...
                $actualPriceR['p'][] = intval(floatval($seq2[$i][1]))%100;
                ...
                $actualPriceI['p'][] = $inc_imaginary%100;
        ...
        for ($x = 0; $x < 14 ; ) // += $day_cnt)
        {
            $j = 0; {
                if ($x == 0)
                    ...
                    $key = $vals;
                $inc_real = $seq2[152 - $x][1];
                array_pop($vals);
                $vals[] = $this->integrand($key);
                $c = 2; //$this->differential($key);
                $vals[2] = $this->integral($seq2[array_key_last($seq2) - 1]);
                array_pop($seq2);
                $vals[3] += 8;

                $actualPriceR['f'][] = $inc_real%100;
                ...
                $actualPriceI['f'][] = abs(($inc_imaginary - $short_low))%100;
                $inc_imaginary = $short_low;
                $inc_last = $inc_real;
        ...
        $actualPriceR['p'] = array_slice($actualPriceR['p'],-21,21,true);
        $actualPriceR['p'] = array_reverse($actualPriceR['p']);
        $actualPriceI['p'] = array_slice($actualPriceI['p'],-21,21,true);
        $actualPriceI['p'] = array_reverse($actualPriceI['p']);
        return json_encode(['r' => ($actualPriceR), 'i' => $actualPriceI]);
    }
}
$ticker = '';
...
if ($ticker == "btc-usd" || !file_exists('./tickers/' . $ticker . '.csv') || filemtime('./tickers/' . $ticker . '.csv') > time() - (60 * 60 * 24 * 5)) {
